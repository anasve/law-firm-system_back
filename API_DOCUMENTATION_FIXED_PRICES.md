...
هذا الملف يوثق جميع الـ API endpoints الخاصة بنظام الأسعار الثابتة (Fixed Prices) في النظام. يسمح النظام للعملاء بعرض الأسعار الثابتة النشطة فقط، بينما يمكن للموظفين إدارتها بالكامل (إنشاء، تحديث، أرشفة، حذف).

This document describes all API endpoints for the Fixed Prices system. The system allows clients to view only active fixed prices, while employees can fully manage them (create, update, archive, delete).
...
الموظفون مسؤولون عن إدارة الأسعار الثابتة بالكامل (إنشاء، تحديث، أرشفة، حذف).
Employees are responsible for fully managing fixed prices (create, update, archive, delete).
...
GET /api/employee/fixed-prices?type=fee
GET /api/employee/fixed-prices?is_active=true
GET /api/employee/fixed-prices?search=ترجمة
GET /api/employee/fixed-prices?type=stamp&is_active=true&per_page=20
```
...
**Endpoint:** `GET /api/employee/fixed-prices/active`
...
**Endpoint:** `GET /api/employee/fixed-prices/archived`
...
**Endpoint:** `GET /api/employee/fixed-prices/{id}`
...
**Endpoint:** `POST /api/employee/fixed-prices`
...
**Endpoint:** `PUT /api/employee/fixed-prices/{id}`
...
**Endpoint:** `DELETE /api/employee/fixed-prices/{id}`
...
**Endpoint:** `PUT /api/employee/fixed-prices/{id}/restore`
...
**Endpoint:** `DELETE /api/employee/fixed-prices/{id}/force`
...
### 1. Employee Creates Fixed Price
```http
POST /api/employee/fixed-prices
...
### 3. Employee Views All Fixed Prices
```http
GET /api/employee/fixed-prices
Authorization: Bearer {employee_token}
```

**Response:** List of all fixed prices with pagination

---

### 4. Employee Updates Price
```http
PUT /api/employee/fixed-prices/1
Authorization: Bearer {employee_token}
Content-Type: application/json
...
### 5. Employee Archives Price
```http
DELETE /api/employee/fixed-prices/1
Authorization: Bearer {employee_token}
```
...
### 6. Employee Restores Price
```http
PUT /api/employee/fixed-prices/1/restore
Authorization: Bearer {employee_token}
```
...
3. **Contact for Updates**: اتصل بالموظف إذا بدت الأسعار قديمة
   Contact employee if prices seem outdated
...
2. **Authorization**: فقط الموظفون يمكنهم إنشاء، تحديث، أو حذف الأسعار
   Only employees can create, update, or delete fixed prices
3. **Soft Deletes**: الأسعار تستخدم soft deletes للحفاظ على سجل التدقيق
   Prices use soft deletes to maintain audit trail
4. **Active Filter**: العملاء يمكنهم رؤية الأسعار النشطة فقط
   Clients can only see active prices

---

## Notes

1. **Active Prices Only**: العملاء يمكنهم عرض الأسعار النشطة فقط
   Clients can only view active fixed prices
...
### Example Request (Employee - Create)
```http
POST {{base_url}}/api/employee/fixed-prices
Authorization: Bearer {{employee_token}}
Content-Type: application/json
